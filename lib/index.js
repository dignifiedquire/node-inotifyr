...
var Inotifyr = module.exports = function (dir, options) {
    ...
    // Used to relate delete and move_to events
    this._deleteJar = {};
...
Inotifyr.prototype._addRecursiveWatches = function (dir, events, initial, callback) {
    var self = this;
    // we do not process symlink now and it can contains circular reference same
    // express@1.0.7 -> ./../../../express/1.0.7 for example
...
Inotifyr.prototype._createDataHandler = function (dir, events, callback) {
    ...
    return function (data) {
        ...
        case 'delete':
            if (!isDir) {
                self._rememberDelete(fullPath);
            }

            self._emitSafe('delete', fullPath, stat);
            break;
        case 'delete_self':
        case 'move_self':
        ...
        case 'move_to':
            ...
            if (!isDir && self._rememberedDelete(fullPath)){
                self.emit('modify', fullPath, stat);
            } else {
                self.emit(eventType, fullPath, stat);
...
// Start the delete cleaner
Inotifyr.prototype._startDeleteCleaner = function () {
    var self = this;
    ...
    this._cleaner = setInterval(function () {
        ...
        _.forEach(self._deleteJar, function (timestamp, key) {
            if (current - timestamp > 50) {
                self._deleteJar[key] = null;
            }
        });
    }, 50);
};

// Remember a delete file
//
// file - String, path to the file.
Inotifyr.prototype._rememberDelete = function (file) {
    this._deleteJar[file] = +new Date();

    this._startDeleteCleaner();
};

// Try to remember a delete operation to match it to a move op
//
// file - String, path to the file
//
// Returns a number if found.
Inotifyr.prototype._rememberedDelete = function (file) {
    var delCookie = this._deleteJar[file];

    if (delCookie) this._deleteJar[file] = null;
